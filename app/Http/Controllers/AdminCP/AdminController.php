...
class AdminController extends Controller
{
  ...
  public function postSample(DemoRequest $request)
  {
   ...
   public function lstQLNS(){
    ...
    if($ma_quyen != 1){
      ...
      $template['users'] = DB::select('select *, IF((ADDTIME(created_at, (select concat(giatrithamso,":00:00") from thamso where id = 3)) < NOW()) and trangthai = 0, "Hết hạn", "") as stt from nhanvien where trangthai in (0,1) and parent_id = '.$nv_id);
    }
    else {
      $template['users'] = DB::select('select *, IF((ADDTIME(created_at, (select concat(giatrithamso,":00:00") from thamso where id = 3)) < NOW()) and trangthai = 0, "Hết hạn", "") as stt from nhanvien where trangthai in (0,1)');
    }
  ...
  public function delAll(Request $request){
    ...
    foreach($arrUser as $user){
      $sqlUpdate = "update nhanvien set trangthai = 3, updated_at = NOW() where id = $user";
